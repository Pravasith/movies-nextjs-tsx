...
const MovieDetails: FC<Props> = (props: Props) => {
    ...
    const returnIMDBStars = (imdb_rating: number) => {
        ...
        const convertTo5Star = Math.ceil((imdb_rating * 10) / 2 / 5) * 5 + ""
        ...
        return starArray.map((item, i) => (
            <div className={`${styles.starWrap}`} key={"stars" + i}>
                {item}
            </div>
        ))
    }
    ...
    return (
        <>
            ...
            <div className={`${styles.container}`}>
                <div
                    className={`${styles.detailsWrap} ${utilStyles.flexRow_NW}`}
                >
                    <div
                        className={`${styles.posterWrap} ${utilStyles.flexRow_Centre}`}
                    >
                        <img src={movieData.poster} alt={movieData.title} />
                    </div>

                    <div
                        className={`${styles.movieDetails} ${utilStyles.flexCol_NW}`}
                    >
                        <div
                            className={`${styles.titleWrap} ${utilStyles.flexRow_NW}`}
                        >
                            <div
                                className={`${styles.leftWrap} ${utilStyles.flexCol_NW}`}
                            >
                            ...
                            <div
                                className={`${styles.rightWrap} ${utilStyles.flexRow_NW}`}
                            >
                                {returnIMDBStars(movieData.imdb_rating)}
                            </div>
                        </div>
                        <div
                            className={`${styles.descriptionWrap} ${utilStyles.flexCol_NW}`}
                        >
                            {
                                <p>
