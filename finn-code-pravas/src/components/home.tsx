...
const Home: FC<Props> = (props: Props) => {
    ...
    const genreWiseData = () => {
        const moviesCategorisedIntoGenres = movies.reduce(
            (all: Genre, movie, i) => {
    ...
    useEffect(() => {
        ...
        Object.keys(genreWiseMovies).forEach(genre => {
            let sliderElement = document.getElementsByClassName(
                `${genre}-slideMovie`
            )[0]
            ...
            if (containerWidth > elementWidth) {
                gsap.set([`.left-btn-${genre}`, `.right-btn-${genre}`], {
                    display: "none",
                })
            }
        })
    ...
    const slideIt = (leftOrRight: string, genre: string) => {
        ...
        if (leftOrRight === "left") {
            ...
            if (noOfScrolls[genre] >= 0) {
                ...
                gsap.set(`.right-btn-${genre}`, {
                    display: "block",
                })

                if (noOfScrolls[genre] === 0) {
                    gsap.set(`.left-btn-${genre}`, {
                        display: "none",
                    })
                }
            } else {
            ...
            if (noOfScrolls[genre] < scrollTimes) {
                ...
                gsap.to(`.${genre}-slideMovie`, {
                    x: -(
                        (scrollTimes - 1) * containerWidth +
                        scrollWidthRemaining
                    ),
                    duration: 0.5,
                    ease: "Power4.out",
                })
            ...
            return (
                ...
                >
                    <a>
                        ...
                        >
                            <div
                                className={`${styles.thumbnail} ${utilStyles.posRel}`}
                            >
                                <div className={`${styles.imgWrap}`}>
                                    {useImage([backdrop, title, `backdrop`])}
                                </div>

                                <div
                                    className={`${styles.titleWrap}  ${utilStyles.posAbs_SW}`}
                                >
                            ...
                            <div
                                className={`${styles.movieDetails} ${utilStyles.flexRow_Centre}`}
                            >
                                <p className={`${styles.genreHashTags}`}>
        ...
        return Object.keys(genreWiseMovies).map((genre, i) => (
            ...
            >
                ...
                <div className={`${styles.sliderContainer}`}>
                    <div
                        className={`${styles.sliderWrap} ${utilStyles.posRel}`}
                    >
                        <div
                            className={`${styles.leftButton} left-btn-${genre} ${utilStyles.posAbs_NW}`}
                        >
                            <button
                        ...
                        <div
                            className={`${styles.rightButton} right-btn-${genre}  ${utilStyles.posAbs_NE}`}
                        >
                            <button
                                className={`${styles.clickButton} ${utilStyles.flexRow_Centre}`}
                                onClick={() => {
